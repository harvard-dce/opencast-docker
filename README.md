...
Opencast comes in different distributions. For each of the official distributions, there is a specific container image. Each version is tagged. For example, the full image name containing the `admin` distribution at version `14.2` is `quay.io/opencast/admin:14.2`. Leaving the version out will install the latest one.
